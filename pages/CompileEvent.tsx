...
const CompileEvent: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-zinc-50 font-sans text-zinc-900">
            {/* ... nav ... */}
            <nav className="p-6 md:p-8 flex items-center gap-4 bg-white border-b border-zinc-100 sticky top-0 z-50">
                <button onClick={() => navigate(-1)} className="w-10 h-10 rounded-full bg-zinc-100 flex items-center justify-center hover:bg-zinc-200 transition-colors">
                    <ArrowLeft size={18} />
                </button>
                <div>
                    <h1 className="text-xl font-black tracking-tight">Compile Video</h1>
                    <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest">{eventName}</p>
                </div>
            </nav>
            ...
            <main className="max-w-2xl mx-auto p-6 md:p-12">
                {!isComplete ? (
                    ...
                    <div className="bg-white p-8 rounded-[40px] shadow-xl border border-zinc-100">
                        {/* ... icons/text ... */}
                        <div className="w-20 h-20 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center mx-auto mb-8">
                            <Film size={32} />
                        </div>
                        <h2 className="text-3xl font-black text-center mb-4">Ready to Create Movie?</h2>
                        ...
                        {!compiling ? (
                            <div className="space-y-6">
                                ...
                                <div className="bg-zinc-50 p-6 rounded-3xl border border-zinc-100">
                                    <h3 className="font-bold mb-4 flex items-center gap-2"><Settings size={18} /> Notification Settings</h3>
                                    <div className="space-y-3">
                                        <label className="flex items-center gap-3 cursor-pointer">
                                            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${notifyEmail ? 'bg-black border-black text-white' : 'border-zinc-300'}`}>
                                                {notifyEmail && <CheckCircle size={14} />}
                                            </div>
                                            <input type="checkbox" className="hidden" checked={notifyEmail} onChange={() => setNotifyEmail(!notifyEmail)} />
                                            <span className="font-medium">Email me when ready</span>
                                        </label>
                                        <label className="flex items-center gap-3 cursor-pointer">
                                            <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${notifyPhone ? 'bg-black border-black text-white' : 'border-zinc-300'}`}>
                                                {notifyPhone && <CheckCircle size={14} />}
                                            </div>
                                            <input type="checkbox" className="hidden" checked={notifyPhone} onChange={() => setNotifyPhone(!notifyPhone)} />
                                            <span className="font-medium">Text me when ready</span>
                                        </label>
                                    </div>
                                </div>
                                ...
                                <button
                                    ...
                                    className="w-full bg-black text-white py-5 rounded-3xl font-black text-xl shadow-xl hover:scale-[1.02] transition-transform disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                            ...
                            <div className="text-center py-8">
                                <div className="w-full bg-zinc-100 rounded-full h-4 mb-4 overflow-hidden">
                                    <div className="h-full bg-blue-600 transition-all duration-300" style={{ width: `${progress}%` }}></div>
                                </div>
                                <p className="font-bold text-zinc-400 uppercase tracking-widest text-sm">Processing... {Math.round(progress)}%</p>
                            </div>
                        )}
                    </div>
                ) : (
                    <div className="bg-white p-8 rounded-[40px] shadow-xl border border-zinc-100 text-center animate-in zoom-in duration-500">
                        ...
                        <h2 className="text-4xl font-black mb-4">It's Ready!</h2>
                        <p className="text-zinc-500 mb-8 font-medium">Your compiled video has been generated successfully.</p>

                        <div className="aspect-video bg-black rounded-3xl mb-8 flex items-center justify-center text-white overflow-hidden shadow-2xl relative group">
                        ...
                        <div className="flex gap-4">
                            <button
                                ...
                                className="flex-1 bg-zinc-100 py-4 rounded-2xl font-bold hover:bg-zinc-200 transition-colors flex items-center justify-center gap-2"
                            >
                                <Play size={20} /> Preview All
                            </button>
                            <button
                                onClick={() => forceDownload(submissions[0], `Wishyoua_Event_${eventId}.mp4`)}
                                disabled={downloading}
                                className="flex-1 bg-black text-white py-4 rounded-2xl font-bold shadow-lg hover:scale-105 transition-transform flex items-center justify-center gap-2 disabled:opacity-50"
                            >
                                {downloading ? <Loader2 className="animate-spin" /> : <Download size={20} />} Download
                            </button>
                        </div>
                    </div>
                )}
